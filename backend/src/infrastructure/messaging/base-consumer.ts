...
export abstract class BaseConsumer implements OnModuleInit {
  ...
  constructor(
    ...
    this.maxRetries =
      options.maxRetries ?? parseInt(process.env.RABBITMQ_RETRY_MAX ?? "5", 10);
    this.backoffBaseMs =
      options.backoffBaseMs ??
      parseInt(process.env.RABBITMQ_BACKOFF_BASE_MS ?? "1000", 10);
  }
  ...
  async onModuleInit(): Promise<void> {
    try {
      const prefetch =
        ...
        this.configService.get<number>("app.rabbitmq.prefetch") ??
        50;
      ...
      while (retries > 0 && (!channel || channel.closed)) {
        try {
          ...
          this.logger.warn(
            `Error getting channel (${retries} attempts left):`,
            err.message,
          );
        }
        ...
        if (!channel || channel.closed) {
          this.logger.warn(
            `Waiting for queue ${queue} to be ready (${retries} attempts left)...`,
          );
          await new Promise((resolve) => setTimeout(resolve, 1000));
        }
        retries--;
      ...
      if (!channel || channel.closed) {
        this.logger.error(
          `Failed to get open channel for queue ${queue} after retries. Workers will not start, but API will continue.`,
        );
        return;
      }
      ...
      try {
        ...
        this.logger.warn(
          `Failed to set prefetch, continuing anyway:`,
      ...
      await channel.consume(queue, async (msg: any) => {
        ...
        try {
          ...
          const attempts =
            content.attempts ||
            parseInt((msg.properties.headers?.attempts as string) || "0", 10);

          this.logger.log(
            `Processing message: ${content.eventType} (messageId: ${messageId}, attempts: ${attempts})`,
          );
          ...
          if (
            !(await this.idempotencyService.acquireLock(messageId, traceId))
          ) {
            this.logger.warn(
              `Message ${messageId} is already being processed, requeue`,
            );
          ...
          try {
            ...
            this.logger.log(
              `Message ${messageId} processed successfully in ${duration}ms`,
            );
          } catch (error) {
            ...
            if (newAttempts >= this.maxRetries) {
              this.logger.error(
                `Message ${messageId} exceeded max retries, moving to DLQ`,
              );
              await this.moveToDLQ(channel, msg, content, error as Error);
            } else {
              const delay = this.calculateBackoff(newAttempts);
              this.logger.warn(
                `Message ${messageId} failed (attempt ${newAttempts}/${this.maxRetries}), requeue with delay ${delay}ms`,
              );
      ...
      this.logger.error(
        `Failed to start consumer for queue ${this.options.queue}:`,
        error,
      );
    }
  }
