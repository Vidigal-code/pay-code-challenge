...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
    ...
    async assertQueue(queue: string): Promise<void> {
        ...
        try {
            ...
            if (error.code === 406) {
                this.logger.warn(`Queue ${queue} exists with different config, will use existing`);
                try {
                    ...
                    if (checkError.code === 404) {
                        ...
                        this.logger.warn(`Cannot check queue ${queue}:`, checkError.message);
                    }
                }
                ...
                if (this.channel) {
                    try {
        await this.channel.assertQueue(queue, {durable: true});
                        this.logger.log(`Queue ${queue} created/verified after channel recreation`);
                    } catch (retryError: any) {
                        if (retryError.code === 406) {
                            this.logger.log(`Queue ${queue} exists, will use existing`);
                        } else {
                            this.logger.error(`Failed to assert queue ${queue} after channel recreation:`, retryError.message);
                        }
                    }
                }
            } else {
    ...
    private setupChannelHandlers(): void {
        ...
        this.channel.on('error', (err: any) => {
            if (err.code === 406) {
                ...
                this.logger.warn(`This is expected when queue exists with different config. Channel will be recreated if needed.`);
            } else {
    ...
    async assertQueueWithOptions(queue: string, options: any): Promise<void> {
            ...
            if (error.code === 406) {
                ...
                try {
                    ...
                    if (queueInfo.messageCount > 0) {
                        this.logger.warn(`Queue ${queue} has ${queueInfo.messageCount} messages, skipping recreation`);
                        return;
                    }
                    await this.channel.deleteQueue(queue, {ifEmpty: true});
                } catch (deleteError: any) {
                    if (deleteError.code === 406 || deleteError.message?.includes('closed')) {
                        this.logger.warn(`Cannot delete queue ${queue}, using existing queue`);
                        await this.ensureChannel();
                        return;
                    }
                }
        ...
        await this.channel.assertQueue(queue, {durable: true, ...(options || {})});
                ...
                this.logger.error(`Failed to assert queue ${queue} with options:`, error.message);
            }
        }
    }
    ...
    async assertEventQueue(queue: string, dlq: string): Promise<void> {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        if (dlqError.code === 406) {
                            this.logger.log(`DLQ ${dlq} exists with different config, will use existing`);
                        } else if (dlqError.message?.includes('closed')) {
                            this.logger.warn(`Channel closed while creating DLQ ${dlq}, will retry`);
                            attempts++;
                            await new Promise(resolve => setTimeout(resolve, 500));
                            continue;
                        } else {
                            this.logger.warn(`Failed to assert DLQ ${dlq}:`, dlqError.message);
                        }
                    }
                    ...
                    if (createError.code === 406) {
                        ...
                        this.logger.warn(`Channel closed while creating queue ${queue}, retrying...`);
                        attempts++;
                        await new Promise(resolve => setTimeout(resolve, 500));
                        continue;
                    } else {
                        this.logger.error(`Failed to create queue ${queue}:`, createError.message);
                        attempts++;
                        await new Promise(resolve => setTimeout(resolve, 500));
                        continue;
                    }
                ...
                this.logger.error(`Error in assertEventQueue for ${queue} (attempt ${attempts + 1}/${maxAttempts}):`, error.message);
                attempts++;
        ...
        this.logger.error(`Failed to assert queue ${queue} after ${maxAttempts} attempts`);
    }
