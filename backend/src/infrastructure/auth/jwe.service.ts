...
export class JWEService {
    ...
    constructor(private readonly configService: ConfigService) {
        const secret = this.configService.get<string>("app.jwe.secret") || "your-256-bit-secret-key-must-be-32-chars-long!!";
    ...
    async encrypt(payload: JWEPayload): Promise<string> {
        ...
        const jwe = await new jose.EncryptJWT(payload as unknown as jose.JWTPayload)
            ...
            .setIssuedAt()
            .setExpirationTime(this.configService.get<string>("app.jwt.expiresIn") || "7d")
            .encrypt(secret);
