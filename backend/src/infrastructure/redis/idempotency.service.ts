...
export class IdempotencyService {
    ...
    constructor(private readonly configService: ConfigService) {
        const url = this.configService.get<string>("app.redisUrl") || process.env.REDIS_URL || "redis://localhost:6379";
        this.redis = new Redis(url);
        this.processingTtl = parseInt(process.env.REDIS_PROCESSING_TTL_MS ?? "3600000", 10); // 1 hora
        this.processedTtl = parseInt(process.env.REDIS_PROCESSED_TTL_MS ?? "604800000", 10); // 7 dias
    }

    async acquireLock(messageId: string, traceId: string): Promise<boolean> {
        try {
            const key = `processing:${messageId}`;
            const result = await this.redis.set(key, traceId, "PX", this.processingTtl, "NX");
            return result === "OK";
        } catch (error) {
    ...
    async isProcessed(messageId: string): Promise<boolean> {
        try {
            ...
            this.logger.error(`Failed to check processed status for ${messageId}:`, error);
            return false;
        }
    }
    ...
    async getProcessedResult(messageId: string): Promise<string | null> {
        try {
            ...
            this.logger.error(`Failed to get processed result for ${messageId}:`, error);
            return null;
        }
    }
}
