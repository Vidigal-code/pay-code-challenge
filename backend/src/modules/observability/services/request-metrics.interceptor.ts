    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        return next.handle().pipe(
            tap(() => {
                ...
                if (this.metrics.httpRequestCounter && this.metrics.httpRequestDuration) {
                    this.metrics.httpRequestCounter.inc({method, route, status: String(status)});
                    this.metrics.httpRequestDuration.observe({method, route, status: String(status)}, durationSeconds);
                }
            }),
        );
    }
