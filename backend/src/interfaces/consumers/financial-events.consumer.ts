    ...
    try {
      ...
      await channel.consume(FINANCIAL_EVENTS_QUEUE, async (msg: any) => {
        ...
        try {
          ...
          if (userId) {
            switch (event) {
              case "transaction.created":
                this.financialEventsGateway.emitTransactionCreated(
                  userId,
                  payload,
                );
                break;
              case "transaction.completed":
                this.financialEventsGateway.emitTransactionCompleted(
                  userId,
                  payload,
                );
                break;
              case "transaction.reversed":
                this.financialEventsGateway.emitTransactionReversed(
                  userId,
                  payload,
                );
                break;
              case "wallet.balance.updated":
                this.financialEventsGateway.emitWalletBalanceUpdated(
                  userId,
                  payload,
                );
                break;
          ...
          this.logger.error(
            `Error processing financial event: ${(error as Error).message}`,
          );
      ...
      this.logger.error(
        `Failed to start financial events consumer: ${(error as Error).message}`,
      );
    }
  }
}
