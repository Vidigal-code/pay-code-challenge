...
export class WalletController {
  ...
  @Get()
  @ApiOperation({
    summary: "Get wallet information for the authenticated user",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Adds funds to the authenticated user's wallet. Supports negative balance - if balance is negative, deposit adds to current value. Minimum amount: 0.01",
  })
  @ApiBody({
    type: DepositDto,
    ...
    examples: {
      example1: {
        ...
        value: { amount: 100.5, description: "Initial deposit" },
      },
      example2: {
        summary: "Deposit with negative balance",
        value: { amount: 50.0, description: "Recovery deposit" },
      },
    },
  })
  @ApiResponse({
  ...
  @ApiResponse({
    status: 400,
    type: ErrorResponse,
    description: "Invalid amount (must be > 0)",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Transfers funds from authenticated user to another user. Validates sufficient balance before processing. Minimum amount: 0.01. Cannot transfer to self.",
  })
  @ApiBody({
    type: TransferDto,
  ...
  @ApiResponse({
    ...
    type: ErrorResponse,
    description:
      "Insufficient balance, invalid amount, or cannot transfer to self",
  })
  @ApiResponse({
    status: 404,
    type: ErrorResponse,
  ...
  @ApiOperation({
    ...
    description:
      "Reverses a completed DEPOSIT or TRANSFER transaction. For deposits, subtracts the amount. For transfers, returns money to sender and subtracts from receiver. Can result in negative balance. Only COMPLETED transactions can be reversed.",
  })
  @ApiBody({
  ...
  @ApiResponse({
    ...
    type: ErrorResponse,
    description:
      "Transaction cannot be reversed (not COMPLETED or already reversed)",
  })
  @ApiResponse({
    status: 404,
    type: ErrorResponse,
  ...
  @ApiOperation({
    ...
    description:
      "Returns paginated list of transactions. Supports filtering by type (DEPOSIT, TRANSFER, REVERSAL) and status (PENDING, COMPLETED, REVERSED, FAILED).",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Returns financial KPIs including total balance, deposits, transfers, received amounts, and transaction statistics. Supports date range filtering (default: last 30 days).",
  })
  @ApiResponse({
    status: 200,
      ...
      properties: {
        kpis: {
          ...
          properties: {
            ...
            totalTransfers: {
              type: "number",
              description: "Total sent in transfers",
            },
            totalReceived: {
              type: "number",
              description: "Total received in transfers",
            },
            totalTransactions: {
              type: "number",
            ...
            completedTransactions: {
              type: "number",
              description: "Completed transactions count",
            },
            failedTransactions: {
              type: "number",
              description: "Failed transactions count",
            },
            reversedTransactions: {
              type: "number",
              description: "Reversed transactions count",
            },
          },
        },
      },
