...
export class FinancialEventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    @WebSocketServer()
    server!: Server;
    ...
    async handleConnection(client: Socket) {
        try {
            ...
            const payload = await this.jwtService.verifyAsync(token, {
                secret: this.configService.get<string>("app.jwt.secret") || process.env.JWT_SECRET,
            });
            ...
            this.logger.warn(`Client ${client.id} disconnected: ${(error as Error).message}`);
            client.disconnect();
        }
    }
    ...
    handleSubscribe(@ConnectedSocket() client: Socket) {
        ...
        if (userId) {
            ...
            this.logger.log(`Client ${client.id} subscribed to financial events for user ${userId}`);
        }
    }
